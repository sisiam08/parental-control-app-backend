...
export class AppUsageController {
  ...
  async getUsage(req: AuthenticatedRequest, res: Response, next: NextFunction) {
    try {
      ...
      const usages = await appUsageService.getAppUsages(childId, date as string | undefined);
  ...
  async toggleBlock(req: AuthenticatedRequest, res: Response, next: NextFunction) {
    try {
      ...
      const block = await appUsageService.toggleAppBlock(
        childId,
        appName,
        packageName,
        Boolean(isBlocked),
        dailyLimitMinutes !== undefined ? Number(dailyLimitMinutes) : undefined
      );
  ...
  async setLimit(req: AuthenticatedRequest, res: Response, next: NextFunction) {
    try {
      ...
      const limit = await appUsageService.setScreenTimeLimit(
        childId,
        Number(dayOfWeek),
  ...
  async filterDomain(req: AuthenticatedRequest, res: Response, next: NextFunction) {
    try {
      const childId = req.params.childId;
      ...
      const filter = await appUsageService.blockWebsite(
        childId,
        domain,
        Boolean(isBlocked),
        category
